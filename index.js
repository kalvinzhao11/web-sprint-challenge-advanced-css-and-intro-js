...
function get20s(array){
  ...
  artist = []
  for (x in array){
    year = artists[x].years.split(" - ");
    if (year[0] >= 1900 && year[1] >= 1900 && year[0] <= 2000 && year[1] <= 2000){
      artist.push(array[x].name)
    }
  }
  return artist
...
console.log(addArtist(artists, 21, "kalvin zhao", "1998 - 2100", "software dev","Chinese-American", "hi im me.  Hi I like to code."))
...
function getHTML(data){
    ...
    for (x in data){
      ...
      console.log(`</div>`)
      console.log(`<div class = "bio">${data[x].bio}</div>`)
      console.log(`</div>`)
    }
  }
getHTML(artists)
